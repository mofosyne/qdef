...
// bytes*, never a missing *namespace declaration* -- so a namespace
// declared on only one code is a single point of failure `resolveStack`
// cannot recover from, unlike the Split-protected content itself.
//
// `resolveStack` (wrappers.js) reads each code's root Record namespace
// (§3.1's ordinary namespace-pairing prefix -- there is no separate
// container discriminator anymore, see docs/DESIGN.md), requires every
// code that declares one to agree, and applies the agreed namespace to
// whatever Record the stack ultimately resolves to -- including one only
// reachable after full Split reassembly.
...
function buildCodes({ repeatNamespaceOnEveryCode }) {
  ...
  // Each fragment is written flat at the root -- the code's own
  // namespace is just that Record's ordinary namespace-pairing prefix
  // (§3.1), not a separate discriminator item.
  return fragmentRecords.map((fragRec, i) => {
    const localNamespace = repeatNamespaceOnEveryCode || i === 0 ? NAMESPACE : undefined;
    return core.encodeContainer({ ...fragRec, localNamespace });
  });
}
...
test('codes disagreeing on the declared namespace is rejected, not silently resolved one way or the other', () => {
  ...
  const codes = [
    core.encodeContainer({ ...fragmentRecords[0], localNamespace: Buffer.from('11111111', 'hex') }),
    core.encodeContainer({ ...fragmentRecords[1], localNamespace: Buffer.from('22222222', 'hex') }),
  ];
...
// "inconsistent namespaces." Fixed via header.namespaceEquals (content
// comparison for Buffers -- namespace is always a byte string now, §3.1,
// so there's no other shape to compare against).
// ---------------------------------------------------------------------
...
test('codes disagreeing on a decentralized namespace (different bytes, same length) are still correctly rejected', () => {
  ...
  const fragmentRecords = wrappers.splitEncode(innerBytes, { count: 2 });
  const codes = [
    core.encodeContainer({ ...fragmentRecords[0], localNamespace: Buffer.from('a9d6e1f30b7c4482', 'hex') }),
    core.encodeContainer({ ...fragmentRecords[1], localNamespace: Buffer.from('ffffffffffffffff', 'hex') }),
  ];
