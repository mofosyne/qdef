...
- `src/lib.rs` — `Container::parse`, the `Records` iterator (key-0 routing,
  missing-key-0 detection), `check_criticality` (even/odd rule, layered on
  top as Record-Type-specific handling per §3.3 — not part of the
  mandatory core itself), `find_value` (field lookup).
